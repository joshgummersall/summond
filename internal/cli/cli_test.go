...
func TestLogsDefaultShowsLastRunOnly(t *testing.T) {
	...
	// Write "previous run" output to the log files.
	...
	// Record execution start — this captures the current file sizes as offsets.
	if err := app.store.RecordExecutionStart(spec.Name, time.Now()); err != nil {
		t.Fatalf("RecordExecutionStart() error = %v", err)
	}

	// Append "current run" output after the recorded offset.
